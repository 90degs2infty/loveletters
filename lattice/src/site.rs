...
#[serde(untagged)]
pub enum Site<O> {
    /// An occupied (populated) site.
    Occupied(O),
    /// A vacant (unpopulated) site.
    #[serde(skip_serializing)]
    Vacant,
}

impl<O> Site<O> {
    /// Check whether this [`Site`] is vacant.
    ...
    /// Turn this [`Site`] into an [`Option`].
    ///
    /// [`Site::Occupied`] is mapped to `Some`, [`Site::Vacant`] to `None`.
...
impl<O: Clone + Debug> Site<O> {
    /// Create a [`Strategy`] generating only [`Site::Vacant`].
    pub fn prop_vacant() -> impl Strategy<Value = Self> {
        Just(Self::Vacant)
    }

    /// Create a [`Strategy`] wrapping values from `o` [`Site::Occupied`].
...
where
    Self: Sized + Strategy<Value = O>,
{
    /// Wrap values generated by this [`Strategy`] in [`Site::Occupied`].
...
where
    ...
    D: Debug,
{
    /// Wrap values generated by the specified [`Strategy`]s as present and correct.
    ...
    /// Wrap values generated by the specified [`Strategy`]s as present and defect.
...
where
    ...
    D: Debug,
{
    /// Create a new [`Strategy`] constantly generating the specified value marked as present and correct.
...
where
    Self: Sized + Strategy<Value = C>,
{
    /// Wrap values generated by this [`Strategy`] in [`Site::Occupied`] and [`Occupation::Correct`].
...
where
    Self: Sized + Strategy<Value = D>,
{
    /// Wrap values generated by this [`Strategy`] in [`Site::Occupied`] and [`Occupation::Defect`].
